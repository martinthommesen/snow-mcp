...
| T7 | Schema cache leaks fields across users | user-aware cache key (userId + content-addressed roleHash); field masking | ✅ **wired** `cache/schema.ts` `roleHash()` → `SchemaCache` identity via `handlers.resolveRoleHash`; `"*"`-collision fixed · `schema-cache.test.ts`, `actor-and-policy.test.ts` (P6b-2) |
| T8 | Replay of request/executor call | leveled idempotency (host); executor DB-unique-index nonce | ✅ host ledger `do/mutation-ledger.ts`→`guardMutation` · `mutation-wiring.test.ts`, `do-partition.test.ts` (S17); 🟢-P8 executor `x_mcp_nonce` unique-index race-close (INSERT-as-arbiter; DB enforcement provable only live) |
| T10 | Dynamic Worker cost explosion | multi-dim atomic reserve-before-load + ENFORCED per-run row/byte caps + daily admission/accrual | ✅ `do/budget.ts` (mutexed global+per-user) + `sn/run-budget.ts` (`countRows`/`countBytes` throw `budget_exceeded`) · `run-budget.test.ts` (S14, P5) |
...
| T14 | OAuth flow abuse | exact redirect_uri, state/nonce, PKCE S256, signed consent state | ✅ **wired** — MCP-client provider (`@cloudflare/workers-oauth-provider`, `allowPlainPKCE:false`); signed single-use consent nonce in OAUTH_KV (`servicenow-auth-handler.ts`, `requireOAuthKv` fail-closed) · `auth-surface.test.ts`. Per-user SN OAuth PKCE: ✅ source `auth/servicenow-{ticket,callback-handler}.ts` · `servicenow-ticket.test.ts`, `servicenow-callback.test.ts`; 🟢-P8 the live authorize/callback dance |
...
| T17 | **integration_user over-reads** | ActorPolicy before every RPC (restrictive policy opt-in) | ✅ host enforcement `authz/actor-policy.ts` `loadActorPolicy` (permissive default; restrictive when `ACTOR_POLICY_*` set) · `actor-and-policy.test.ts`; 🟢-P8 live read-policy on the instance (B5; earlier live proof predates the restrictive loader) |
| T18 | `scriptedRest` bypasses executor | path denylist | ✅ `sn/scripted-rest-denylist.ts` · `url-and-path-guards.test.ts` (B2; helper available, no generic `scriptedRest` method to wire — see DELTAS deferred B2) |
| T19 | **ServiceNow-side egress** via runServerScript | tenant toggle + approval + non-recoverable label + host audit/ledger/snapshot wrap | ✅ host controls wired `sn/mutation-guard.ts` (audit/ledger/approval on `runServerScript`; signed+audited `reason`) · `mutation-wiring.test.ts` (P4); 🟢-P8 executor kill-switch/egress live behavior (SNOW_EGRESS.md) |
| T20 | Recovery snapshot store = 2nd sensitive DB | retention + KEK + PII class + opt-out | ✅ **store built + wired** `recovery/snapshots.ts` → `SNAPSHOT_KV` (30-day TTL, sealed under versioned `SNAPSHOT_KEK` ring, fail-closed before mutate) · `audit-recovery.test.ts`, `mutation-wiring.test.ts`; policy in RETENTION.md (P4) |
...
`GlideDigest` SHA-256 UTF-8 encoding (0.13a), and the `x_mcp_nonce` unique-index DB enforcement
