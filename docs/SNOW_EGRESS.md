...
1. **Tenant kill switches** — properties `x_1793136_mcp.executor.enabled=false` and
   ...
   upgrades re-arm both scoped toggles disabled.
...
Production executor use must pass the production posture preflight.
...
- **Executor defaults OFF** — fresh installs and upgrades set the scoped `enabled` and
  `run_server_script_enabled` properties to `false`; the runtime defaults fail closed if either
  scoped property is absent.
